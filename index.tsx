...
`;
  ...
  const processFiles = async () => {
    if (!videoFile || !pdf1File || !pdf2File) {
      alert("Please upload Video and both PDF files.");
      return;
    }

    setIsAnalyzing(true);
    setStatus('Processing files simultaneously (Parallel Mode)...');
    setResults([]);

    try {
      // 1. Performance Optimization: Parallel Processing
      const [pdf1Images, pdf2Images, videoFrames] = await Promise.all([
        extractPdfImages(pdf1File),
        extractPdfImages(pdf2File),
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto font-sans">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Dual Deck Video Slide Analyzer (One Deck Logic)</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="p-4 bg-red-50 rounded-lg border border-red-100">
          <label className="block text-sm font-semibold text-red-900 mb-2">2. First Deck (PDF 1)</label>
          <input 
            type="file" 
        ...
        <div className="p-4 bg-green-50 rounded-lg border border-green-100">
          <label className="block text-sm font-semibold text-green-900 mb-2">3. Second Deck (PDF 2)</label>
          <input 
            type="file" 
            accept="application/pdf" 
      ...
      <div className="mb-8 text-center">
        <button
          ...
          disabled={isAnalyzing || !videoFile || !pdf1File || !pdf2File}
        ...
        >
          {isAnalyzing ? 'Analyzing...' : 'Start Dual Deck Analysis'}
        </button>
